...
A VGG-19_bn network is used in this project. It is pretrained on the ImageNet-Dataset. The parameters in the feature block of the network is fixed and setted as ''requires no grad''. The classifier block is reconstructed to match the task. The network is fine tuned on the in course given dataset.
