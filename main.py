...
if __name__ == '__main__':
    ...
    trainer.test('checkpoints/checkpoint_0076_0.714.ckp')'''

    '''# Train VGG19 with unbalanced data
    train_dataset = pd.read_csv('data_train_unbalanced.csv', sep=';')
    train_dl = CAPTCHADataset(train_dataset, 'train')

    valid_dataset = pd.read_csv('data_valid_unbalanced.csv', sep=';')
    valid_dl = CAPTCHADataset(valid_dataset, 'valid')
    ...
    trainer.fine_tune(80, 'checkpoints/checkpoint_0088_0.693.ckp', lr=0.0005)'''
    ...
    trainer.test_vote(
        [
            'checkpoints/checkpoint_0066_0.713.ckp',
            'checkpoints/checkpoint_0076_0.714.ckp',
            'checkpoints/checkpoint_0088_0.693.ckp'
        ],
        [VGGnet(fine_tuning=True, num_classes=12),
         VGGnet(fine_tuning=True, num_classes=12),
         VGGnet(fine_tuning=True, num_classes=12)]
    )
